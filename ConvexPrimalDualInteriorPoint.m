function ConvexPrimalDualInteriorPoint(c,Q,A,b,Name)
...
%% function definition
alpha = @(v, dv, u, du) min ([1, min(-v(dv<0)./dv(dv<0)), min(-u(du<0)./du(du<0))]);

alpha_max = @(v, dv) min (1, min(-v(dv<0)./dv(dv<0)));

y = @(x, z, n) x'*z/n;
yaff = @(x, ax, dx, z, az, dz, n )  (x + ax* dx)'*(z+az*dz)/n;
tau = @(yaff, y) (yaff/y)^3;
rp = @(A,x,b) -(A*x -b);
rd = @(A,pi,z,c,Q, x) -(A' * pi + z -c -Q'*x);
...
t = table(0,x0(1),x0(2),x0(3), c'*x0, pi0(1),pi0(2),...
    z0(1),z0(2),z0(3), b'*pi0, 0 ,sc0(1), sc0(2), sc0(3));
t.Properties.VariableNames = ...
    {'K','x1','x2','x3','cTx','\pi1','\pi2','z1','z2','z4','bTpi',...
    '\tau','||Ax-b||','||A\pi+z-c||','xz'};
...
for K = 1:itr
    ...
    alpha_aff_0 = alpha(x0,dx_aff_0,z0,dz_aff_0);

    
    y0 = y(x0,z0,n);
    
    y0_aff = yaff (x0,alpha_aff_0,dx_aff_0,z0,alpha_aff_0,dz_aff_0,n);
    ...
    alpha_max_x = alpha_max(x0,dx_0);
    alpha_max_z = alpha_max(z0, dz_0);
    
    
    alpha_0 = min ([1, etha*alpha_max_x, etha*alpha_max_z]);
    
    
    
    xk = x0 + alpha_0 * dx_0;
    pik = pi0 + alpha_0 * dpi_0;
    zk = z0 + alpha_0 * dz_0;
    ...
    tk = table(K, xk(1),xk(2),xk(3), ...
        c' * xk, pik(1),pik(2), zk(1),zk(2),zk(3),...
